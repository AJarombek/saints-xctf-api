...
def test_route_auth(
    case: TestCase, client: FlaskClient, verb: str, endpoint: str, variant: AuthVariant
) -> None:
    """
...
async def get_jwt_token(
    test_suite: Union[TestSuite, Any], auth_url: str, client_id: str, client_secret: str
) -> None:
    """
...
def create_andy2_test_user(case: TestCase) -> None:
    ...
    response: Response = case.client.get(
        "/v2/users/andy2", headers={"Authorization": f"Bearer {case.jwt}"}
    )
