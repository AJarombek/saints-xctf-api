...
activation_code_route = Blueprint(
    "activation_code_route", __name__, url_prefix="/v2/activation_code"
)
...
def activation_code_post() -> Response:
    ...
    code_added_successfully: bool = ActivationCodeDao.add_activation_code(
        new_code=code_to_add
    )

    if code_added_successfully:
        code_added: Code = ActivationCodeDao.get_activation_code(
            code=code_to_add.activation_code
        )
...
def activation_code_by_code_soft_delete(code: str) -> Response:
    ...
    if is_deleted:
        response = jsonify(
            {"self": f"/v2/activation_code/soft/{code}", "deleted": True}
        )
        response.status_code = 204
        return response
    else:
